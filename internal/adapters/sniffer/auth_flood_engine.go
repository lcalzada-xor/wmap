...
type AuthFloodController struct {
	...
	mu       sync.RWMutex
	injector *Injector // Dedicated injector for this attack
}
...
func NewAuthFloodEngine(injector *Injector, locker ChannelLocker, maxConcurrent int) *AuthFloodEngine {
	if maxConcurrent <= 0 {
		maxConcurrent = 5
	}
...
func (e *AuthFloodEngine) runAttack(ctx context.Context, controller *AuthFloodController, injector *Injector) {
	action := func() error {
		defer func() {
			controller.mu.Lock()
